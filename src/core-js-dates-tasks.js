...
function getNextFriday(date) {
  ...
  const daysNumber = currentDate.getDay();
  let daysToNextFriday = (5 - daysNumber + 7) % 7;
  if (daysToNextFriday === 0) {
    daysToNextFriday = 7;
  }
  currentDate.setDate(currentDate.getDate() + daysToNextFriday);
  return currentDate;
}
...
function getCountDaysInMonth(month, year) {
  const dateOfNextMonth = new Date(year, month, 1);
  dateOfNextMonth.setDate(dateOfNextMonth.getDate() - 1);
  const lastDayOfMonth = dateOfNextMonth.getDate();
...
function formatDate(date) {
  ...
  const formattedDate = `${month}/${day}/${year}, ${hour}:${minute.toString().padStart(2, '0')}:${second.toString().padStart(2, '0')} ${amOrPm}`;
...
function getWeekNumberByDate(date) {
  ...
  const weekNumber = Math.ceil((diff + 1 + dayOfWeek) / 7);
  return weekNumber;
}
...
function getNextFridayThe13th(date) {
  ...
  if (currentDate === 13 && date.getDay() === 5) {
